...
const useGeneratePodcast = ({setAudio, voiceType, voicePrompt, setAudioStorageId}: GeneratePodcastProps) => {
  ...
  const generatePodcast = async () => {
    ...
    if(!voicePrompt){
      toast({
        title: "Need a voiceType to generate a podcast"
      })
      return setIsGenerating(false);
    }

    try{
      // send make reques to openai
      const response = await getPodcastAudio({
        voice: voiceType,
        input: voicePrompt
      })
      console.log("got reponse back from openai");
      // saving audio into file
      ...
      toast({
        title: "Audio clip generated successfully"
      })
    ...
    <div>
      <div className="flex flex-col gap-2.5">
        ...
        <Textarea
          ...
          placeholder="write your text here to generate audio"
          rows={5}
